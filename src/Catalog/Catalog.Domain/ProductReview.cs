...
public class ProductReview : AuditableEntity
{
    ...
    private ProductReview(Guid productId, string userId, int rating, string? body) : base()
    {
        ProductId = productId;
        UserId = userId;
        Rating = rating;
    ...
    public static ProductReview Create(Guid productId, string userId, int rating, string? body)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(userId);
        if (rating is < 1 or > 5)
            throw new ArgumentException("Rating must be 1–5", nameof(rating));
        return new ProductReview(productId, userId, rating, body);
    }
