...
if (!app.Environment.IsEnvironment("Test"))
{
    ...
    recurringJobManager.AddOrUpdate<IMediator>("mature-funds", mediator => mediator.Send(new MatureFundsCommand(Guid.NewGuid().ToString()), CancellationToken.None), Cron.Hourly);
}
app.Run();
