...
public class PayoutsWebAppFactory : WebApplicationFactory<Program>, IAsyncLifetime
{
    ...
    public async Task EnsureSchemaAsync() { await using var scope = Services.CreateAsyncScope(); var db = scope.ServiceProvider.GetRequiredService<PayoutsDbContext>(); await db.Database.EnsureDeletedAsync(); await db.Database.OpenConnectionAsync(); try { await db.Database.ExecuteSqlRawAsync("CREATE SCHEMA IF NOT EXISTS payouts;"); } finally { await db.Database.CloseConnectionAsync(); } await db.Database.EnsureCreatedAsync(); }
